...
public class MEConfigUtil {
    ...
    private final String TAG_MAX_STACK_SIZE = "MinStackSize";
    private final String TAG_WHITE_LIST = "WhiteList";
    private final String TAG_BLACK_LIST = "BlackList";
    ...
    public void writeMaxStackSize(CompoundTag tag, int minStackSize) {
        tag.putInt(TAG_MAX_STACK_SIZE, minStackSize);
    }

    public void readMaxStackSize(CompoundTag tag, Consumer<Integer> setter) {
        if (tag.contains(TAG_MAX_STACK_SIZE)) {
            setter.accept(tag.getInt(TAG_MAX_STACK_SIZE));
        }
    }

    public void writeWhiteList(CompoundTag tag, String whiteList) {
        tag.putString(TAG_WHITE_LIST, whiteList);
    }

    public void readWhiteList(CompoundTag tag, Consumer<String> setter) {
        if (tag.contains(TAG_WHITE_LIST)) {
            setter.accept(tag.getString(TAG_WHITE_LIST));
        }
    }

    public void writeBlackList(CompoundTag tag, String blackList) {
        tag.putString(TAG_BLACK_LIST, blackList);
    }

    public void readBlackList(CompoundTag tag, Consumer<String> setter) {
        if (tag.contains(TAG_BLACK_LIST)) {
            setter.accept(tag.getString(TAG_BLACK_LIST));
        }
    }
}
