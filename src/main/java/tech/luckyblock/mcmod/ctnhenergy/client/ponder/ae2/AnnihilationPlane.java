...
public class AnnihilationPlane {
    ...
    public static void annihilationPlane(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.showText(60,
                "The annihilation plane will collect blocks or items in front of it into the network (if possible)",
                "破坏面板会将其前面的方块或者掉落物收集到网络中（如果能的话）")
                .attachKeyFrame();
        scene.idle(60);
    ...
    public static void filter(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.showText(60, "You'll find that the annihilation plane collects everything in front of it...",
                "你会发现破坏面板会收集它前方的所有物品……")
        ...
        scene.world().modifyEntity(item1, e -> e.kill());
        scene.idle(40);
        scene.showText(60, "Note that blocks without drops will always be broken by the annihilation plane",
                "需要注意的是，没有掉落物的方块无论怎样都会被破坏面板破坏")
                .attachKeyFrame();
        scene.idle(80);
        scene.showText(60, "Enchanting the annihilation plane with Silk Touch may help solve this issue",
                "为破坏面板附魔精准采集可能可以为解决该问题提供思路")
                .attachKeyFrame();
        scene.idle(60);
    }
}
